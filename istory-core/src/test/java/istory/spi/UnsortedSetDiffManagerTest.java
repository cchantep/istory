 ...
 */
public class UnsortedSetDiffManagerTest extends TestCase {
	...
	for (; services.hasNext(); i++) {
	    ...
	    assertTrue("Found provider should accept filtered class",
		       spi.accept(Set.class));

	    if (!spi.accept(SortedSet.class)) {
		m = spi;
	    } // end of if
	} // end of for
	...
	assertFalse("Provider should not accept sorted set",
		    m.accept(SortedSet.class));
...
} // end of class UnsortedSetDiffManagerTest
